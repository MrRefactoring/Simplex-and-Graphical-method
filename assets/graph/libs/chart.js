    ...
    return function e(t, n, r) {
        ...
        6: [function(require, module, exports) {
            ...
            module.exports = function(chart) {
                let minWidthHeight;
                let xScale = chart.meta.xScale;
                let yScale = chart.meta.yScale;

                function clampRange(vLo, vHi, gLo, gHi, closed) {
                    if (gLo > gHi) {
                        let t = gLo;
                        gLo = gHi;
                        gHi = t
                    ...
                    let hi, lo;
                    if (closed === 'top'){  // Если надо заполнить область над ф-ей
                        hi = Math.min(vHi, gHi);
                        lo = Math.min(vLo, gLo);
                    } else if (closed === 'bottom'){  // Если надо заполнить область под ф-ей
                        hi = Math.max(vHi, gHi);
                        lo = Math.max(vLo, gLo);
                    } else {  // Выделяем область интеграла
                        hi = Math.min(vHi, gHi);
                        lo = Math.max(vLo, gLo);
                    }
                ...
                let line = function(points, closed) {  // todo
                    let path = "";
                    ...
                    for (let i = 0, length = points.length; i < length; i += 1) {
                        if (points[i]) {
                            ...
                            let yLo = y.lo;
                            let yHi = y.hi;
                            if (typeof closed === 'boolean' && closed) {  // todo
                                yLo = Math.min(yLo, 0);
                                yHi = Math.max(yHi, 0);
                            }
                            let moveX = xScale(x.lo) + points.scaledDx / 2;
                            let viewportY = clampRange(minY, maxY, isFinite(yHi) ? yScale(yHi) : -Infinity, isFinite(yLo) ? yScale(yLo) : Infinity, closed);
                            let vLo = viewportY[0];
                            let vHi = viewportY[1];
                            path += " M " + moveX + " " + vLo;
                ...
                function plotLine(selection) {
                    selection.each(function(d) {
                        let el = plotLine.el = d3.select(this);
                        let index = d.index;
                        let closed = d.closed;  // todo
                        let evaluatedData = evaluate(chart, d);
                        let innerSelection = el.selectAll(":scope > path.line").data(evaluatedData);
                        ...
                        innerSelection.attr("stroke-width", minWidthHeight).attr("stroke", utils.color(d, index)).attr("opacity", closed ? 1 / d.count : d.count).attr("d", function(d) {  // todo
                            return line(d, closed)  // todo
                        }).attr(d.attr);
        ...
        7: [function(require, module, exports) {  // todo
            "use strict";
            let d3 = window.d3;
            let evaluate = require("../evaluate");
            let utils = require("../utils");
            ...
            module.exports = function(chart) {
                ...
                function plotLine(selection) {
                    selection.each(function(d) {
                        ...
                        let line = d3.svg.line().interpolate("linear").x(function(d) {
                            return xScale(d[0])
                        }).y(y);
